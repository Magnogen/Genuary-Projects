...
async function setup() {
  ...
  for (let {x, y, i} of coords) {
    ...
    if (pixels[i] > 200 && dist(x, y, width/2, width/2) > width*0.45) {
      pixels[i + 0] = pixels[i + 1] = pixels[i + 2] = 220
    } 
  ...
  for (let {x, y, i} of coords) {
    ...
    if (pixels[i] > 200) {
      if (random() < 0.0002) {
        if (dist(x, y, width/2, height/2) > width*0.45) continue
        pixels[i + 0] = col.r
        pixels[i + 1] = col.g
        pixels[i + 2] = col.b
  ...
  for (let {x, y, i} of coords) {
    ...
    if (dist(x, y, width/2, width/2) > width*0.45) {
      pixels[i + 0] = pixels[i + 1] = pixels[i + 2] = 220
    } 
